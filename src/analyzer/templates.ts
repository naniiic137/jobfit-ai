...
export const T = {
  ...
  summaryNoRequired: { en: ' No required skill was recognised in this ad.', fr: ' Aucune compétence requise n’a été reconnue dans cette annonce.' },
  summaryLowCoverage: {
    en: ' Only {n} skill(s) of this ad are in the offline skills list, so treat the score as a rough hint.',
    fr: ' Seulement {n} compétence(s) de cette annonce figurent dans la liste hors ligne : ce score n’est qu’une indication.',
  },
  ...
  qGapWhy: { en: '{skill} is required and the keyword scan did not find it in your CV — expect them to probe it.', fr: '{skill} est requis et l’analyse par mots-clés ne l’a pas trouvé dans votre CV — attendez-vous à des questions dessus.' },
  qGap: { en: 'This role uses {skill}. What have you done with it so far, and how would you get up to speed in your first weeks?', fr: 'Ce poste utilise {skill}. Qu’avez-vous déjà fait avec, et comment vous mettriez-vous à niveau les premières semaines ?' },
  qGapTip: { en: 'If it is new to you, say so honestly. Give a concrete plan (docs, a small project, pairing) and connect it to something similar you know{related}.', fr: 'Si c’est nouveau pour vous, dites-le franchement. Donnez un plan concret (doc, petit projet, binôme) et reliez-le à ce que vous connaissez déjà{related}.' },
...
export const QUESTION_BANK: Record<string, Dict> = {
  ...
  testing: { en: 'What do you unit-test versus integration-test? Show a test you are proud of.', fr: 'Que testez-vous en unitaire et en intégration ? Montrez un test que vous jugez particulièrement réussi.' },
...
export const SOFT_QUESTIONS: Record<string, Dict> = {
  ...
  autonomy: { en: 'Describe a time you were blocked and had to find the answer on your own.', fr: 'Décrivez une situation de blocage où vous avez dû trouver la solution par vous-même.' },
...
export const COVER = {
  en: {
    ...
    opening: (title: string | null, company: string | null) =>
      `I am writing to apply for the ${title ?? 'advertised'} position${company ? ` at ${company}` : ''}.`,
    ...
    // Growth-oriented on purpose: a keyword miss does not prove the candidate never used it.
    gaps: (skills: string, plural: boolean) =>
      `The role also involves ${skills}; ${plural ? 'these are areas' : 'this is an area'} I am keen to keep developing, and I learn new tools best by building with them.`,
  ...
  fr: {
    // The standard, gender-neutral French opening; no company name after it.
    greeting: (_company: string | null) => 'Madame, Monsieur,',
    opening: (title: string | null, company: string | null) =>
      `Je vous adresse ma candidature pour ${title ? `le poste de ${title}` : 'le poste proposé'}${company ? ` chez ${company}` : ''}.`,
    stack: (skills: string, years: number | null) =>
    ...
    gaps: (skills: string, plural: boolean) =>
      `Le poste implique aussi ${skills} : ${plural ? 'ce sont des domaines dans lesquels' : 'c’est un domaine dans lequel'} je souhaite continuer à progresser, et j’apprends le mieux en construisant.`,
    soft: (skills: string, plural: boolean) =>
      `L’annonce mentionne aussi ${skills} ; ${plural ? 'ces qualités comptent' : 'cette qualité compte'} autant pour moi que la technique.`,
    closing: (company: string | null) =>
      `Je me tiens à votre disposition pour échanger sur la manière dont je peux contribuer à ${company ?? 'votre équipe'}. Je vous remercie de l’attention portée à ma candidature.`,
